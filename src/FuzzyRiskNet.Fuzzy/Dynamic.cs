...
#pragma warning restore 0219
            ...
            if (token.id == TokenId.OpenParen) {
                ...
                switch (FindMethod(type, id, instance == null, args, out mb)) {
                    ...
                    case 1:
                        ...
                        if (instance != null && method.DeclaringType == typeof(Math)) // Added by Niknejad for direct access to Math
        ...
        int FindMethod(Type type, string methodName, bool staticAccess, Expression[] args, out MethodBase method) {
            ...
            var types = SelfAndBaseTypes(type).AsQueryable();
            ...
            if (staticAccess || types.Contains(typeof(DynamicClass))) // Added by Niknejad to include direct access to Math class 
            {
                MemberInfo[] members = typeof(Math).FindMembers(MemberTypes.Method,
                    BindingFlags.Public | BindingFlags.Static | BindingFlags.DeclaredOnly, Type.FilterNameIgnoreCase, methodName).ToArray();//GetExtensionMethods
                int count = FindBestMethod(members.Cast<MethodBase>(), args, out method);
                if (count != 0) return count;
            }
            method = null;
